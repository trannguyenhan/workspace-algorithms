...
/**
*
* Kiểm tra đẳng cấu giữa G và H qua song ánh (hay hoán vị) p
*
**/
bool check (vector<int> &p, Graph &G, Graph &H)
{
  ...
  // Kiểm tra nếu dãy bậc bằng nhau 
...
/**
 * Hàm chính: Kiểm tra hai đồ thị có đẳng cấu hay không 
 * */
bool check_isomorphism_graph (Graph &G, Graph &H)
{
  ...
  //Duyệt mọi hoán vị để kiểm tra đẳng cấu  
  do
  {
      if (check (p, G, H ))
      {
          // In ra song ánh (hay hoán vị) tìm được 
          for(int i: p)
            cout<<i<<" ";
          cout<<endl;
          return true;
